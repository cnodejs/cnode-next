...
Cloudflare Web Analytics RUM is installed by the Web application when `CNODE_WEB_RUM_TOKEN` is set. Store only the public beacon token in the deployment dotenv file; the SSR script receives the response nonce. Keep Cloudflare automatic HTML injection disabled so Web remains the sole owner of document scripts and CSP. After enabling RUM, inspect its browser requests and add only the required Cloudflare origins to `connect-src`.
