...
Schema changes start in Drizzle definitions. Generate and review migration files for shared or production databases; direct push is limited to disposable local development databases.

```mermaid
flowchart LR
  Schema[packages/db/src/schema] --> Generate[pnpm db:generate]
  Generate --> Files[packages/db/migrations/pg]
  Schema --> Push[pnpm db:push:pg local only]
  Files --> Pg[(PostgreSQL)]
  Push --> Pg
```
...
- Apply reviewed migrations explicitly and create a verified restorable backup before destructive or irreversible changes.
