...
message.openapi(listMessagesRoute, async (c) => {
  ...
  const [readMsgs, unreadMsgs] = await Promise.all([getReadMessagesByUserId(currentUser.id), getUnreadMessagesByUserId(currentUser.id)]);
  const formatMessage = async (msg: any) => {
    ...
    return {
      ...
      author: relations.author ? { loginname: relations.author.loginname, avatar_url: relations.author.avatar } : { loginname: "", avatar_url: "" },
      topic: relations.topic ? { id: String(relations.topic.id), author: relations.topic.author ? { loginname: relations.topic.author.loginname, avatar_url: relations.topic.author.avatar } : { loginname: "", avatar_url: "" }, title: relations.topic.title, last_reply_at: relations.topic.lastReplyAt } : null,
      reply: relations.reply ? { id: String(relations.reply.id), content: renderMarkdown(relations.reply.content, mdrender), ups: [], create_at: relations.reply.createAt } : {},
    };
  };
  const [hasReadRaw, hasUnreadRaw] = await Promise.all([Promise.all(readMsgs.map(formatMessage)), Promise.all(unreadMsgs.map(formatMessage))]);
  ...
  return c.json({ success: true as const, data: { has_read_messages: hasRead, hasnot_read_messages: hasUnread } }, 200);
});

// GET /message/count
const messageCountRoute = createRoute({
  method: "get", path: "/message/count", tags: ["messages"], summary: "获取未读消息数",
  request: { query: z.object({ accesstoken: z.string().optional() }) },
  responses: { 200: { description: "未读数", content: { "application/json": { schema: z.object({ success: z.literal(true), data: z.number() }) } } }, 401: { description: "未登录", content: { "application/json": { schema: errorResponseSchema } } } },
});
...
const markAllRoute = createRoute({
  method: "post", path: "/message/mark_all", tags: ["messages"], summary: "标记全部已读",
  request: { body: { content: { "application/json": { schema: markAllBodySchema } } } },
  responses: { 200: { description: "成功", content: { "application/json": { schema: z.object({ success: z.literal(true), marked_msgs: z.array(z.object({ id: z.string() })) }) } } }, 401: { description: "未登录", content: { "application/json": { schema: errorResponseSchema } } } },
});
message.openapi(markAllRoute, async (c) => {
  ...
  await updateMessagesToRead(currentUser.id, unread.map((m) => m.id));
  return c.json({ success: true as const, marked_msgs: unread.map((m) => ({ id: String(m.id) })) }, 200);
});

// POST /message/mark_one/:msg_id
const markOneRoute = createRoute({
  method: "post", path: "/message/mark_one/{msg_id}", tags: ["messages"], summary: "标记单条已读",
  request: { params: msgIdParamSchema, body: { content: { "application/json": { schema: markOneBodySchema } } } },
  responses: { 200: { description: "成功", content: { "application/json": { schema: z.object({ success: z.literal(true), marked_msg_id: z.string() }) } } }, 401: { description: "未登录", content: { "application/json": { schema: errorResponseSchema } } } },
});
