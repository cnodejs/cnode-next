...
const presignUploadSchema = z.object({
  ...
  contentType: z
    .string()
    .regex(/^image\/(png|jpeg|gif|webp|svg\+xml)$/)
    .default("image/png"),
});
...
function staticUploadUrl(filename: string) {
  return `${(process.env.OSS_STATIC_HOST || "https://static.cnodejs.org").replace(/\/+$/g, "")}/${filename}`;
}
...
function webBaseUrl() {
  return process.env.CNODE_WEB_BASE_URL || "http://localhost:5173";
}
...
function clearPendingGithubProfile(c: any) {
  deleteCookie(c, GITHUB_PENDING_COOKIE, {
...
function getPendingGithubProfile(c: any): PendingGithubProfile | null {
  ...
  try {
    ...
    return p.id && p.login && p.email && p.accessToken ? p : null;
  } catch {
    return null;
  }
}
function getGithubOAuthState(c: any): GithubOAuthState | null {
  ...
  try {
    ...
    return p.state && ["login", "bind"].includes(p.intent) ? p : null;
  } catch {
    return null;
  }
}
function clearGithubOAuthState(c: any) {
  deleteCookie(c, GITHUB_OAUTH_COOKIE, {
    domain: process.env.AUTH_COOKIE_DOMAIN || undefined,
    path: "/",
  });
}
      ...
      content: {
        "application/json": {
          schema: z.object({
            ...
            data: z.object({ allow_signup: z.boolean() }),
          }),
        },
      },
    },
...
auth.openapi(signupRoute, async (c) => {
  ...
  if (await userQueries.getByLoginName(loginname.toLowerCase()))
    return c.json({ success: false as const, error_msg: "用户名已被使用" }, 422);
...
const activeAccountRoute = createRoute({
  method: "get",
  path: "/auth/local/active_account",
  tags: ["auth"],
  summary: "激活账号",
...
auth.openapi(resetPassRoute, async (c) => {
  ...
  if (!user || !user.retrieveTime || Date.now() - Number(user.retrieveTime) > RETRIEVE_KEY_TTL)
    return c.json({ success: false as const, error_msg: "重置链接无效或已过期" }, 400);
...
auth.get("/auth/github/callback", async (c) => {
  ...
  try {
    ...
    const profileRes = await fetch("https://api.github.com/user", {
      headers: { Authorization: `token ${ghAccessToken}`, "User-Agent": "cnode-next" },
    });
    ...
    if (oauthState.intent === "bind") {
      ...
      if (decision === "reject-different" || decision === "reject-occupied")
        return rejectGithubBind(
          ...
          profile.login,
          ghAccessToken,
          decision === "reject-different" ? "github_different_account" : "github_already_bound",
        );
      try {
...
const githubPendingRoute = createRoute({
  method: "get",
  path: "/auth/github/pending",
  tags: ["auth"],
  summary: "获取 GitHub pending profile",
  responses: {
    200: {
...
const githubCreateRoute = createRoute({
  method: "post",
  path: "/auth/github/create",
  tags: ["auth"],
  summary: "GitHub 注册或关联",
  middleware: [perIpPerDay("create_user_per_ip", CREATE_USER_PER_IP, true)],
...
auth.openapi(githubCreateRoute, async (c) => {
  ...
  if (body.isnew) {
    ...
    if (await userQueries.getByLoginName(loginname))
      return c.json(
        { success: false as const, error_msg: "GitHub 用户名已被使用，请关联老账号" },
        422,
      );
    if (await userQueries.getByEmail(profile.email))
      return c.json({ success: false as const, error_msg: "该邮箱已注册，请关联老账号" }, 422);
    try {
      ...
      if (isGithubIdUniqueViolation(error)) {
        ...
        return c.json(
          { success: false as const, error_msg: "该 GitHub 账号已绑定到其他用户" },
          409,
        );
      }
      throw error;
      ...
      return c.json(
        {
          ...
          error_msg:
            decision === "reject-different"
              ? "当前账号已绑定其他 GitHub 账号，请先解绑"
              : "该 GitHub 账号已绑定到其他用户",
        },
        409,
      );
...
auth.openapi(githubUnbindRoute, async (c) => {
  ...
  if (!user.githubId)
    return c.json({ success: false as const, error_msg: "当前账号未绑定 GitHub" }, 409);
  ...
  try {
    ...
    return c.json({ success: false as const, error_msg: "解除绑定暂时失败，请稍后重试" }, 503);
  }
  if (!result.success && result.reason === "invalid-password") {
    ...
    return c.json(
      { success: false as const, error_msg: "当前密码错误；如果你从未设置过密码，请先重置密码" },
      403,
    );
  }
  if (!result.success && result.reason === "revoke-failed") {
    ...
    return c.json(
      { success: false as const, error_msg: "GitHub 授权暂时无法撤销，请稍后重试" },
      503,
    );
  }
  if (!result.success && result.reason === "binding-changed") {
    ...
    return c.json(
      { success: false as const, error_msg: "GitHub 绑定状态已变化，请刷新后重试" },
      409,
    );
  }
  if (!result.success)
        ...
        "application/json": {
          schema: z
            .object({
              ...
              receive_reply_mail: z.boolean().optional(),
              receive_at_mail: z.boolean().optional(),
            })
            .passthrough(),
        },
      },
...
auth.openapi(settingRoute, async (c) => {
  const user = c.get("user");
  if (!user) return c.json({ success: false as const, error_msg: "未登录" }, 401);
  const { url, location, signature, weibo, receive_reply_mail, receive_at_mail } =
    c.req.valid("json");
...
const changePassRoute = createRoute({
  method: "post",
  path: "/auth/local/change_pass",
  tags: ["auth"],
  summary: "修改密码",
  request: {
    body: {
      content: {
        "application/json": { schema: z.object({ oldPass: z.string(), newPass: z.string() }) },
      },
    },
  },
  responses: {
...
auth.openapi(changePassRoute, async (c) => {
  ...
  if (newPass.length < 8 || !/[a-zA-Z]/.test(newPass) || !/[0-9]/.test(newPass))
    return c.json(
      { success: false as const, error_msg: "新密码至少 8 位,必须包含字母和数字" },
      422,
    );
...
auth.openapi(presignRoute, async (c) => {
  ...
  if (!parsed.success)
    return c.json(
      { success: false as const, error_msg: "只支持 png/jpeg/gif/webp/svg 图片上传" },
      422,
    );
