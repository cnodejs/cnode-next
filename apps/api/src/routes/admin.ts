...
const auditCategoryValues = new Set(["content", "user", "role", "account", "security", "moderation", "system"]);
const auditRiskValues = new Set(["low", "medium", "high", "critical"]);
const auditTargetTypeValues = new Set(["topic", "topics", "reply", "user", "ip", "report", "scan_job", "settings", "zone", "tab", "moderation_hit"]);
...
export function auditEventMeta(action: string) {
  ...
  const category = action.startsWith("report_") || action.startsWith("moderation_")
    ? "moderation"
...
function redactSensitive(value: unknown): unknown {
  ...
  for (const [key, child] of Object.entries(value)) {
    result[key] = /password|passwd|secret|token|access[_-]?token|private|credential|database_url/i.test(key) ? "[redacted]" : redactSensitive(child);
  }
  return result;
}
...
export function sanitizeAuditDetail(detail?: string | null) {
  ...
  try {
    ...
    return detail.replace(/(password|passwd|secret|token|access[_-]?token|database_url)(\s*[=:]\s*)\S+/gi, "$1$2[redacted]");
  }
}
...
function buildAuditConditions(c: any) {
  ...
  if (q) conditions.push(or(sql`${auditLogs.action} LIKE ${`%${q}%`}`, sql`${auditLogs.operatorName} LIKE ${`%${q}%`}`, sql`${auditLogs.targetName} LIKE ${`%${q}%`}`, sql`${auditLogs.targetId} LIKE ${`%${q}%`}`, sql`${auditLogs.detail} LIKE ${`%${q}%`}`));
...
function auditSummary(logs: any[]) {
  return logs.reduce((summary, log) => {
    ...
    if (["delete_topic", "delete_reply", "delete_all_user_content", "permanent_delete_topic"].includes(log.action)) summary.content_deletions += 1;
    if (meta.category === "role") summary.role_changes += 1;
    if (meta.category === "account") summary.account_security += 1;
    if (!["success", "confirmed", "dismissed", "created"].includes(String(log.result || ""))) summary.failures += 1;
    return summary;
...
async function permanentlyDeleteTopic(topicId: number) {
  ...
  return db.transaction(async (tx: any) => {
    const replyRows = await tx.select({ id: replies.id }).from(replies).where(eq(replies.topicId, topicId));
    const replyIds = replyRows.map((reply: any) => reply.id);

    if (replyIds.length > 0) {
      await tx.delete(replyUps).where(inArray(replyUps.replyId, replyIds));
      await tx.delete(messages).where(or(eq(messages.topicId, topicId), inArray(messages.replyId, replyIds)));
      await tx.delete(reports).where(or(and(eq(reports.targetType, "topic"), eq(reports.targetId, topicId)), and(eq(reports.targetType, "reply"), inArray(reports.targetId, replyIds))));
      await tx.delete(moderationHits).where(or(eq(moderationHits.topicId, topicId), and(eq(moderationHits.targetType, "topic"), eq(moderationHits.targetId, topicId)), and(eq(moderationHits.targetType, "reply"), inArray(moderationHits.targetId, replyIds))));
      await tx.delete(replies).where(inArray(replies.id, replyIds));
    } else {
      await tx.delete(messages).where(eq(messages.topicId, topicId));
      await tx.delete(reports).where(and(eq(reports.targetType, "topic"), eq(reports.targetId, topicId)));
      await tx.delete(moderationHits).where(or(eq(moderationHits.topicId, topicId), and(eq(moderationHits.targetType, "topic"), eq(moderationHits.targetId, topicId))));
    }
    ...
    const [remainingTopics, remainingReplies, remainingCollects, remainingJobMeta, remainingMessages, remainingHits, remainingReports, remainingReplyUps, remainingReplyMessages, remainingReplyHits, remainingReplyReports] = await Promise.all([
      ...
      tx.select({ c: count() }).from(reports).where(and(eq(reports.targetType, "topic"), eq(reports.targetId, topicId))),
      replyIds.length ? tx.select({ c: count() }).from(replyUps).where(inArray(replyUps.replyId, replyIds)) : Promise.resolve([{ c: 0 }]),
      replyIds.length ? tx.select({ c: count() }).from(messages).where(inArray(messages.replyId, replyIds)) : Promise.resolve([{ c: 0 }]),
      replyIds.length ? tx.select({ c: count() }).from(moderationHits).where(and(eq(moderationHits.targetType, "reply"), inArray(moderationHits.targetId, replyIds))) : Promise.resolve([{ c: 0 }]),
      replyIds.length ? tx.select({ c: count() }).from(reports).where(and(eq(reports.targetType, "reply"), inArray(reports.targetId, replyIds))) : Promise.resolve([{ c: 0 }]),
    ]);
    const remaining = Number(remainingTopics[0]?.c || 0) + Number(remainingReplies[0]?.c || 0) + Number(remainingCollects[0]?.c || 0) + Number(remainingJobMeta[0]?.c || 0) + Number(remainingMessages[0]?.c || 0) + Number(remainingHits[0]?.c || 0) + Number(remainingReports[0]?.c || 0) + Number(remainingReplyUps[0]?.c || 0) + Number(remainingReplyMessages[0]?.c || 0) + Number(remainingReplyHits[0]?.c || 0) + Number(remainingReplyReports[0]?.c || 0);
...
admin.get("/admin/topics", modRequired(), async (c) => {
  ...
  const [list, totalResult] = await Promise.all([listQuery.orderBy(topicOrder(filters.sort), desc(topics.id)).limit(pagination.limit).offset(pagination.offset), totalQuery]);
  return c.json(paginated(list.map((t: any) => ({ id: t.id, title: t.title, tab: t.tab, top: t.top, good: t.good, lock: t.lock, archived: t.archived, status: t.status, deleted: t.deleted, reply_count: t.replyCount, visit_count: t.visitCount, collect_count: t.collectCount, create_at: t.createAt, update_at: t.updateAt, last_reply_at: t.lastReplyAt })), Number(totalResult[0]?.c || 0), pagination));
});

admin.post("/admin/topics/permanent-delete", adminRequired(), async (c) => {
  if (!canRunPermanentTopicDelete(c.get("isAdmin"))) return c.json({ success: false, error_msg: "需要管理员权限" }, 403);
  ...
  for (const id of ids) {
    ...
    if (!result) continue;
    results.push({ topic_id: id, title: result.topic.title, deleted_replies: result.deletedReplies });
  }
  if (!results.length) return c.json({ success: false, error_msg: "话题不存在" }, 404);
  await auditQueries.log(user.id, user.loginname, "permanent_delete_topic", { type: "topics", id: results.map((item) => item.topic_id).join(","), name: results[0]?.title }, "success", JSON.stringify({ topics: results, deleted: results.length }));
  return c.json({ success: true, deleted: results.length, topics: results });
});

admin.post("/admin/topics/:action", modRequired(), async (c) => {
  const action = c.req.param("action");
  if (!canRunTopicAction(action, true, true)) return c.json({ success: false, error_msg: "未知操作" }, 400);
...
admin.post("/admin/moderation/bulk", modRequired(), async (c) => {
  ...
  if (jobId > 0 && !ids.length) {
    if (!canRunJobBulkModerationAction(action, c.get("isAdmin"))) return c.json({ success: false, error_msg: action === "confirm" ? "需要管理员权限" : "任务级批量处理仅支持确认删除" }, action === "confirm" ? 403 : 400);
    const hits = await db.select().from(moderationHits).where(and(eq(moderationHits.scanJobId, jobId), eq(moderationHits.status, "pending")));
    let handled = 0;
    for (const hit of hits) {
      if (await applyModerationHitAction(hit, action as any, user)) handled += 1;
    }
    await auditQueries.log(user.id, user.loginname, `moderation_job_bulk_${action}`, { type: "scan_job", id: String(jobId) }, action, JSON.stringify({ job_id: jobId, handled }));
...
admin.get("/admin/audit", adminRequired(), async (c) => {
  ...
  const pageRows = category || risk
    ? filteredSummaryRows.slice(pagination.offset, pagination.offset + pagination.limit)
    : list;
  return c.json({
    ...paginated(pageRows.map(formatAuditLog), category || risk ? filteredSummaryRows.length : Number(totalResult[0]?.c || 0), pagination),
