...
function publicTopicSql(topicId: any = topics.id) {
  return sql`${boolEq(topics.deleted, false)} and coalesce(${topics.status}, 'published') <> 'deleted' and (${topics.tab} is null or ${topics.tab} not in (${sql.join(
    ...
    sql`, `,
  )})) and exists (select 1 from ${users} where ${users.id} = ${topics.authorId} and ${boolEq(users.isBlock, false)}) and ${topics.id} = ${topicId}`;
}
...
const sidebarHomeRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": {
          schema: z.object({ success: z.literal(true), data: sidebarHomeResponseSchema }),
        },
      },
    },
  },
