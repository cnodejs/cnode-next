...
import { collectBodySchema, collectLoginNameParamSchema, topicDTOSchema, errorResponseSchema } from "@cnode/shared";
...
const listCollectRoute = createRoute({
  method: "get", path: "/topic_collect/{loginname}", tags: ["collections"], summary: "获取用户收藏列表",
  request: { params: collectLoginNameParamSchema },
  responses: { 200: { description: "收藏列表", content: { "application/json": { schema: z.object({ success: z.literal(true), data: z.array(topicDTOSchema) }) } } }, 404: { description: "用户不存在", content: { "application/json": { schema: errorResponseSchema } } } },
});
collect.openapi(listCollectRoute, async (c) => {
  ...
  const db = getDb();
  const collects = await db.select().from(topicCollects).where(eq(topicCollects.userId, userData.id)).limit(100);
...
const collectRoute = createRoute({
  method: "post", path: "/topic_collect/collect", tags: ["collections"], summary: "收藏话题",
  request: { body: { content: { "application/json": { schema: collectBodySchema } } } },
  responses: { 200: { description: "收藏成功", content: { "application/json": { schema: z.object({ success: z.literal(true) }) } } }, 401: { description: "未登录", content: { "application/json": { schema: errorResponseSchema } } }, 404: { description: "话题不存在", content: { "application/json": { schema: errorResponseSchema } } }, 409: { description: "已收藏", content: { "application/json": { schema: errorResponseSchema } } } },
});
collect.openapi(collectRoute, async (c) => {
  ...
  if (!(await isPublicTopic(topicData))) return c.json({ success: false as const, error_msg: "话题不存在" }, 404);
  const db = getDb();
...
const deCollectRoute = createRoute({
  method: "post", path: "/topic_collect/de_collect", tags: ["collections"], summary: "取消收藏",
  request: { body: { content: { "application/json": { schema: collectBodySchema } } } },
  responses: { 200: { description: "取消成功", content: { "application/json": { schema: z.object({ success: z.literal(true) }) } } }, 401: { description: "未登录", content: { "application/json": { schema: errorResponseSchema } } }, 404: { description: "话题不存在或未收藏", content: { "application/json": { schema: errorResponseSchema } } } },
});
collect.openapi(deCollectRoute, async (c) => {
  ...
  const db = getDb();
  const existing = await db.select().from(topicCollects).where(and(eq(topicCollects.userId, user.id), eq(topicCollects.topicId, tid))).limit(1);
  if (existing.length === 0) return c.json({ success: false as const, error_msg: "尚未收藏该主题" }, 404);
  await db.delete(topicCollects).where(and(eq(topicCollects.userId, user.id), eq(topicCollects.topicId, tid)));
