...
function publicTopicExists(topicId: any) {
  return sql`exists (select 1 from ${topics} where ${topics.id} = ${topicId} and ${boolEq(topics.deleted, false)} and coalesce(${topics.status}, 'published') <> 'deleted' and (${topics.tab} is null or ${topics.tab} not in (${sql.join(
    INTERNAL_TABS.map((tab) => sql`${tab}`),
    sql`, `,
  )})) and exists (select 1 from ${users} where ${users.id} = ${topics.authorId} and ${boolEq(users.isBlock, false)}))`;
}
...
const starsRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": {
          schema: z.object({ success: z.literal(true), data: z.array(publicUserSchema) }),
        },
      },
    },
  },
...
const top100Route = createRoute({
  ...
  path: "/users/top100",
  tags: ["users"],
  summary: "获取积分前 100 用户",
  responses: {
    200: {
      description: "Top 100 用户",
      content: {
        "application/json": {
          schema: z.object({ success: z.literal(true), data: z.array(publicUserSchema) }),
        },
...
user.openapi(userRepliesRoute, async (c) => {
  ...
  const topicRows = await db
    ...
    .from(replies)
    .where(
      and(eq(replies.authorId, userData.id), boolEq(replies.deleted, false), visibleTopicExists),
    )
  ...
  const topicList =
    ids.length > 0
      ? await db
          .select()
          .from(topics)
          .where(publicTopicWhere(inArray(topics.id, ids)))
      : [];
...
const userDetailRoute = createRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": {
          schema: z.object({ success: z.literal(true), data: userDetailSchema }),
        },
      },
    },
    404: {
...
user.openapi(userDetailRoute, async (c) => {
  ...
  for (const topic of replyTopics) {
    if (
      ...
      INTERNAL_TABS.includes(topic.tab || "")
    )
      continue;
  ...
  return c.json(
    {
      ...
      data: {
        ...
        is_muted: !!userData.isMuted,
        recent_topics: recentTopics.map(fmtTopic),
        recent_replies: recentRepliesTopics.map(fmtTopic),
      },
    },
    200,
  );
...
const accessTokenRoute = createRoute({
  ...
  path: "/accesstoken",
  tags: ["auth"],
  summary: "验证 accessToken",
  request: {
    body: { content: { "application/json": { schema: z.object({ accesstoken: z.string() }) } } },
  },
  responses: {
    200: {
      description: "验证成功",
    ...
    400: {
      description: "缺少 accesstoken",
      content: { "application/json": { schema: errorResponseSchema } },
    },
    403: {
      description: "accesstoken 无效",
      content: { "application/json": { schema: errorResponseSchema } },
    },
  },
});
user.openapi(accessTokenRoute, async (c) => {
  ...
  if (!accesstoken)
    return c.json({ success: false as const, error_msg: "accesstoken is required" }, 400);
