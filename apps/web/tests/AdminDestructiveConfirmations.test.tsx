...
describe("后台破坏性操作确认", () => {
  ...
  it("uses distinct confirmations for block, mute, roles, credentials, and delete-all", async () => {
    ...
    expect(screen.getByRole("alertdialog", { name: "屏蔽用户内容" })).toHaveTextContent(
      "不等同于禁言",
    );
    await user.click(screen.getByRole("button", { name: "取消" }));
    ...
    expect(screen.getByRole("alertdialog", { name: "禁言用户" })).toHaveTextContent(
      "已有内容不会因此自动隐藏",
    );
    await user.click(screen.getByRole("button", { name: "取消" }));
    ...
    expect(screen.getByRole("alertdialog", { name: "重置用户密码" })).toHaveTextContent(
      "现有密码将立即失效",
    );
    await user.click(screen.getByRole("button", { name: "取消" }));
    ...
    expect(screen.getByRole("alertdialog", { name: "删除用户所有发言" })).toHaveTextContent(
      "用户账号本身不会删除",
    );
    await user.click(screen.getByRole("button", { name: "取消" }));
    expect(apiFetch).not.toHaveBeenCalled();
  ...
  it("requires confirmation before hiding topics", async () => {
    ...
    expect(screen.getByRole("alertdialog", { name: "确认切换话题可见性" })).toHaveTextContent(
      "1 个话题",
    );
    expect(apiFetch).not.toHaveBeenCalled();
    await user.click(screen.getByRole("button", { name: "取消" }));
  ...
  it("keeps a failed IP rule confirmation and its input available for retry", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("IP 或 CIDR (如 1.2.3.4 或 1.2.3.0/24)"),
      "1.2.3.4",
    );
    await user.click(screen.getByRole("button", { name: "添加" }));
    expect(screen.getByRole("alertdialog", { name: "确认添加 IP 封禁规则" })).toHaveTextContent(
      "1.2.3.4",
    );
    expect(apiFetch).not.toHaveBeenCalled();
    ...
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "确认添加 IP 规则" })).toBeEnabled(),
    );
  ...
  it("keeps skipped bulk governance targets selected for retry", async () => {
    ...
    expect(toastSuccess).toHaveBeenCalledWith("解除禁言结果：成功 1 个，跳过 1 个，失败 0 个", {
      description: "可重试目标：2",
    });
  });
