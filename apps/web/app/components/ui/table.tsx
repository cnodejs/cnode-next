...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn("border-t bg-cnode-soft/60 font-medium [&>tr]:last:border-b-0", className)}
    {...props}
  />
));
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "border-b border-border/80 transition-colors hover:bg-cnode-soft/70 data-[state=selected]:bg-cnode-soft",
        className,
      )}
      {...props}
    />
