...
import { TopicTabInfoCard } from "./TopicTabInfo";
...
export function Sidebar({
  ...
  return (
    <aside className="flex flex-col gap-5 md:gap-6">
      <SidebarContextCard tab={tab} label={tabs.find((item) => item.key === tab)?.label} />
...
function SidebarContextCard({ tab, label }: { tab: string; label?: string }) {
  if (tab !== "all" && tab !== "good") {
    return <TopicTabInfoCard tab={tab} label={label} />;
  }
  return (
    <Card size="sm">
      <CardHeader>
      ...
      <CardContent>
        ...
        {replies.map((reply) => {
          ...
          return (
            <Item
              key={reply.id}
              render={<Link to={`/topic/${reply.topic_id}#${reply.id}`} />}
              variant="muted"
              size="xs"
            >
              <ItemMedia>
              ...
              <ItemContent>
                <ItemTitle className="line-clamp-2 w-full text-xs">{reply.topic_title}</ItemTitle>
                <ItemDescription>
  ...
  return (
    <Card size="sm">
      ...
      <CardContent>
        <ItemGroup>
          {topics.map((t) => (
            <Item key={t.id} render={<Link to={`/topic/${t.id}`} />} variant="muted" size="xs">
              <ItemMedia variant="icon">
                <MessageCircle />
              </ItemMedia>
              <ItemContent>
                <ItemTitle className="line-clamp-2 w-full text-xs">{t.title}</ItemTitle>
                <ItemDescription>
                <span>{getTabLabel(t.tab)}</span>
                  {t.create_at ? <> · <TimeAgo date={t.create_at} /></> : null}
                </ItemDescription>
              </ItemContent>
            </Item>
          ))}
