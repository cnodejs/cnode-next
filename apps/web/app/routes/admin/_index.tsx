...
export async function loader({ request }: any) {
  ...
  const [statsRes, recentUsersRes, recentTopicsRes] = await Promise.all([
    apiFetch<{ success: boolean; data: any }>("/api/v1/admin/stats", { headers: { cookie } }).catch(
      () => null,
    ),
    apiFetch<{ success: boolean; data: any[] }>("/api/v1/admin/recent-users", {
      headers: { cookie },
    }).catch(() => null),
    apiFetch<{ success: boolean; data: any[] }>("/api/v1/admin/recent-topics", {
      headers: { cookie },
    }).catch(() => null),
  ]);
  return {
  ...
  return (
    <AdminLayout>
      <AdminPage archetype="dashboard">
        <AdminPageHeader
          title="管理概览"
          description="社区运行状态、近期内容和新用户的统一工作台。"
        />
                ...
                recentUsers.map((u: any) => (
                  <Item key={u.id} size="sm" render={<Link to={`/user/${u.loginname}`} />}>
                    <ItemMedia>
                      <Avatar>
                        <AvatarImage
                          src={u.avatar_url || "https://gravatar.com/avatar/?size=32"}
                          alt=""
                        />
                        <AvatarFallback>
                          {String(u.loginname || "?")
