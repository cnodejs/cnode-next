  ...
  return (
    <AdminLayout>
      <AdminPage archetype="workflow">
        <AdminPageHeader
          title="举报队列"
          description="集中确认用户举报，区分违规内容和误报反馈。"
        />
        <AdminPanel
          title="举报记录"
          description={`当前显示 ${reports.length} / ${total} 条举报记录`}
        >
                  ...
                  <ItemContent>
                    <ItemHeader>
                      <ItemTitle>
                        ...
                        <a href={r.topic_id ? `/topic/${r.topic_id}` : "#"}>
                          {r.topic_title || "目标内容已不可见"}
                        </a>
                      </ItemTitle>
                    </ItemHeader>
                    <ItemDescription>
                      {r.reporter_count || 0} 人举报 · {r.target_type} #{r.target_id}
                    </ItemDescription>
                    {r.target_type === "reply" && (
                    ...
                    {r.description && (
                      <p className="border-l-2 pl-3 text-sm break-words text-muted-foreground">
                        {r.description}
                      </p>
                    )}
                  </ItemContent>
          ...
          <ConfirmationDialog
            ...
            description={
              <>
                将确认举报 #{confirmTarget?.id} 违规，并删除
                {confirmTarget?.target_type === "reply" ? "回复" : "话题"} #
                {confirmTarget?.target_id}：{confirmTarget?.topic_title || "目标内容"}。
              </>
            }
