  ...
  return (
    <AdminLayout>
      <AdminPage>
        ...
        <AdminPanel title="内容治理队列" description={`当前页 ${topics.length} 条 / 共 ${total} 条`}>
          ...
          <AdminToolbar className="items-stretch bg-card/95 p-3 sm:flex-col sm:items-stretch">
            ...
            {selected.length > 0 && (
              <div className="flex flex-wrap items-center gap-2 rounded-2xl border border-cnode-green/20 bg-cnode-soft p-3 text-sm">
                <Badge variant="default">已选 {selected.length}</Badge>
                <Button size="sm" variant="outline" onClick={() => handleAction("top")}>切换置顶</Button>
                {isAdmin && (
                  <>
