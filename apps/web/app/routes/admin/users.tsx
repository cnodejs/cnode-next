...
export async function loader({ request }: any) {
  ...
  const res = await apiFetch<{ success: boolean; data: any[] }>("/api/v1/admin/users?limit=50", {
    headers: { cookie },
  });
...
export default function AdminUsers({ loaderData }: any) {
  ...
  const filtered = users.filter(
    (u) =>
      !search ||
      u.loginname?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <AdminLayout>
      <AdminPage>
        ...
        <AdminPanel title="用户列表" description={`当前显示 ${filtered.length} / ${users.length} 个用户`}>
          <AdminToolbar>
            <Input
              value={search}
