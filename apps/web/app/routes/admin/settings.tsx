...
export async function loader({ request }: any) {
  ...
  const res = await apiFetch<{ success: boolean; data: any }>("/api/v1/admin/settings", {
    headers: { cookie },
  });
...
export default function AdminSettings({ loaderData }: any) {
  ...
  const { run: handleSave, pending: saving } = useAsyncAction(
    () =>
      apiFetch<{ success: boolean; error_msg?: string }>("/api/v1/admin/settings", {
        method: "POST",
  ...
  return (
    <AdminLayout>
      <AdminPage archetype="workflow">
        <AdminPageHeader
          title="系统设置"
          description="维护注册、内容巡检、新用户门槛和基础限流策略。"
        />
        <Tabs
        ...
        >
          <TabsList className="w-full max-w-3xl flex-wrap justify-start">
            <TabsTrigger value="registration" className="flex-none">
              注册配置
            </TabsTrigger>
            <TabsTrigger value="newuser" className="flex-none">
              新用户限制
            </TabsTrigger>
            <TabsTrigger value="rate" className="flex-none">
              限流配置
            </TabsTrigger>
          </TabsList>
              ...
              <FieldSet>
                ...
                <FieldGroup>
                  <Field orientation="responsive">
                    <FieldLabel htmlFor="allow_signup">开放注册</FieldLabel>
                    <Checkbox
                      id="allow_signup"
                      aria-label="开放注册"
