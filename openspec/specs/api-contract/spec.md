...
- **AND** 项目 MUST NOT 保留手写 OAS 或任何与路由声明脱节的 OAS 文件

#### Scenario: OAS 从路由自动生成

- **WHEN** 开发者修改 `apps/api/src/routes/*.ts` 中某个端点的 path、method、zod schema 或 OpenAPI metadata
- **THEN** 运行 `pnpm gen:openapi` MUST 重新生成 `apps/web/public/openapi.json` 反映该变更
- **AND** `pnpm verify` 流程 MUST 在 `verify:openapi` 之前自动执行 `gen:openapi`
- **AND** 生成的 `apps/web/public/openapi.json` MUST 不包含 `x-contract-response-fields` 扩展——响应 schema 由完整 zod 定义描述
...
- **THEN** 生成的 `apps/web/public/openapi.json` MUST 包含该端点
