...
- [ ] 5.2 在测试环境使用两个 GitHub 身份执行绑定、重复绑定、防换绑、密码重置后解绑和重新绑定 smoke
- [ ] 5.3 在生产 migration 前运行重复 `github_id` 只读预检，确认结果为零后应用 migration，并验证唯一索引存在
- [ ] 5.4 部署后在桌面与移动端验收账号身份 UI，并验证解绑成功后 GitHub 登录不再命中原账号、本地密码登录和当前 Session 仍可用
