...
- [x] 2.4 在候选 CSP 中配置报告目标，并通过浏览器 smoke test 验证首页、登录、话题详情、管理入口和 Turnstile 相关页面在 Report-Only 下可渲染、hydration、导航、请求 API 和加载当前头像来源。

## 3. Feature-Complete：部署与强制切换

- [x] 3.1 核实生产 Caddy 和其他入口的现有 header 配置，为 CSP、HSTS 及其他安全头指定唯一所有者，并确认最终客户端响应不存在冲突、重复或策略放宽。
