...
定义本变更涉及数据库 schema、生产必需配置、seed 脚本的安全边界，防止非临时数据库被误清空或被未经 review 的 push 直接修改。

## ADDED Requirements

### Requirement: 非临时数据库只通过 Drizzle migration 发布

非临时数据库的 schema 变更 SHALL 通过已生成、已 review 的 Drizzle migration 文件执行。`drizzle-kit push` SHALL 仅用于本地临时开发库快速验证，不得作为发布路径。
