...
- [x] 1.3 更新 Button、Badge、Card、DropdownMenu、Sheet、Dialog、Table、Skeleton、Toast、Avatar 样式，使其使用新的品牌 surface 和交互 tokens。
...
- [x] 2.5 将 public、content、auth、search、topic、user、message 和 admin routes 中的临时宽度替换为正确的命名模板。
...
- [x] 5.6 验证 `reply2` 通知行为正确，且不会重复通知 topic 作者。
...
- [x] 7.5 统一 toast、loading、empty、error、disabled、hover、active、focus、selected 状态在 routes 和 components 中的表现。

## 8. 后台一致性

- [x] 8.1 确保所有 admin routes 使用 admin template、共享 Header cluster、active navigation、CNode brand tokens 和一致内容宽度。
- [x] 8.2 使用与主站相同的 card/line/shadow 层级重设计 admin cards、tables、filters、dialogs 和 empty/error states。
- [x] 8.3 验证 admin 移动端布局使用可用的横向或折叠导航，并且内容无 overflow。
...
- [x] 9.3 以约 1280px 桌面宽度审计 `/`、`/topic/:id`、`/topic/create`、`/search`、`/user/:name`、`/my/messages`、`/signin`、`/signup`、`/about`、`/faq`、`/getstart`、API 文档页和所有 admin routes。
- [x] 9.4 以约 390px 移动端宽度审计同一批 route families，重点检查 Header、search/cmd、sidebars/rails、TOC、forms、footer 和 touch targets。
- [x] 9.5 检查审计 routes 的浏览器 console errors，并修复非预期 runtime errors。
- [x] 9.6 验证所有可见且 enabled 的控件都会执行其宣称的 action 或 navigation，包括评论回复、发布、messages、profile menus、admin nav、search/cmd 和 footer links。
- [x] 9.7 验证 topic list、topic detail、comments、sidebar、leaderboard、profile 和 messages 中的图片与头像都有可用 URL 或确定性 fallback。
- [x] 9.8 验证代表性组件的 hover、active、focus-visible、disabled、selected、loading、empty 和 error states。
- [x] 9.9 在标记实现完成前记录最终 self-audit 范围、检查页面、viewport 覆盖、运行命令、已知 gaps 和 residual risks。
