...
系统 SHALL 在切换线上流量前通过 nodeclub online replacement 验收矩阵，证明公开 URL、API 契约、业务副作用、生产周边行为和容器运行方式均可替代旧站。生产验收 MUST 确认部署不会执行镜像构建，并且 API、Web 和 worker 必须来自已通过 release verification gate 的不可变镜像发布物。
