...
- **AND** workflow MUST 推送 `ghcr.io/cnodejs/cnode-api:sha-<commit>` 或记录等价 image digest
- **AND** workflow MUST NOT 在 release verification gate 失败时推送镜像

#### Scenario: Web 镜像发布到 GHCR SHA tag
...
- **AND** 运维 MUST 执行 `docker compose -f deployment/docker-compose.yml up -d --no-build api web worker`
...
- **THEN** 系统 MUST 使用运行时 `.env` 注入到页面的 `APP_API_BASE_URL`
